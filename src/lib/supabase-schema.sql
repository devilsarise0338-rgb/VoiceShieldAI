...
-- Storage Policy: Users can only upload and read files in their own folder (folder name = user_id)
CREATE POLICY "User storage read access" ON storage.objects
  FOR SELECT USING (bucket_id IN ('voice-reference-audio', 'analysis-audio', 'reports') AND (auth.uid())::text = (storage.foldername(name))[1]);

CREATE POLICY "User storage upload access" ON storage.objects
  FOR INSERT WITH CHECK (bucket_id IN ('voice-reference-audio', 'analysis-audio', 'reports') AND (auth.uid())::text = (storage.foldername(name))[1]);

CREATE POLICY "User storage delete access" ON storage.objects
  FOR DELETE USING (bucket_id IN ('voice-reference-audio', 'analysis-audio', 'reports') AND (auth.uid())::text = (storage.foldername(name))[1]);
